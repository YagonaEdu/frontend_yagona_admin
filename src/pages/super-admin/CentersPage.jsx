...
export default function CentersPage() {
  ...
  async function load() {
    setError("");
    try {
      const list = await listPlatformTenants();
      setTenants(list);
      if (selected) {
  ...
  useEffect(() => {
    if (!openWizard && !selected) return undefined;
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Клиенты платформы: поиск, карточка и отдельная форма подключения."
        actions={
      ...
      <FiltersBar>
        <div className="grid cols-3" style={{ gap: 10, alignItems: "end" }}>
          <Field label="Поиск">
            <input
              value={query}
                  ...
                  <div className="grid cols-2" style={{ gap: 12 }}>
                    <Field label="Тариф">
                      <select value={form.plan} onChange={(e) => setField("plan", e.target.value)}>
                        <option value="trial">Пробный — 14 дней</option>
          ...
          <aside className="sheet sheet-detail">
            ...
            <div className="sheet-body">
              <div className="detail-badges">
                ...
                />
                <span className="status">{productsOf(selected)}</span>
              </div>
              ...
              <div className="detail-quick">
                ...
                <div>
                  <span>Договор</span>
                  <strong>{selected.contract_number || "—"}</strong>
                </div>
                <div>
